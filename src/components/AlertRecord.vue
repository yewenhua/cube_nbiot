        ...
        <div class="filter">
            <ul>
                ...
                <li @click="showType" :class="second ? 'active' : '' ">
                    <div class="diy">
                        <div class="inner">{{selectedType.text ? selectedType.text : '报警类型'}}</div>
                    </div>
                </li>
            ...
            <div class="inner-content">
                <ul>
                    <li v-for="(item, index) in datalist" :key="index">
                        <div class="left">
                            <img :src="item.type == '防撬' ?  qiao_icon : item.type == '钥匙' ? key_icon : item.type == '低电量' ? lower_icon : forbidden_icon"/>
                        </div>
                        <div class="right">
...
//                    value: unixtimefromat((new Date()).getTime()).date
                ...
                third: false,
                toast: null,
                qiao_icon: require('../assets/img/fangqiao.svg'),
                key_icon: require('../assets/img/key.svg'),
                forbidden_icon: require('../assets/img/forbidden.svg'),
                lower_icon: require('../assets/img/lowerpower.svg')
            }
        },
        methods: {
            showToastLoading () {
            ...
            getLogData(cb){
                if(!this.loading) {
                    checkToken(function () {
                        ...
                        this.loading = true;
                        let url = '/v1/alarm/list/' + this.selectedDevice.id;
                        this.axios({
                            method: 'post',
                            url: url,
                            data: {
                                searchMap: {
                                    type_eq: [this.selectedType.value],
...
<style scoped="scoped">
    ...
    .inner-content ul li .right .item.time{
        color: #898D96;
        font-size: 0.12rem;
        font-weight: normal;
    }
