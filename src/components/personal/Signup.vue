    ...
    <div class="wechat-page">
        ...
        <div class="w-content">
            <div class="input-item has-border">
                ...
                <div class="right">
                    <cube-input v-model="password" type="password" class="bind-input" placeholder="请输入登陆密码"></cube-input>
                </div>
            </div>
            <div class="input-item has-border">
                <div class="left">
                ...
                <div class="right">
                    <cube-input v-model="repassword" type="password" class="bind-input" placeholder="请输入确认密码"></cube-input>
                </div>
            </div>
        </div>

        <div class="bind">
            <cube-button :primary="true" @click="sbmit">提交注册</cube-button>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods:{
                ...
                if (that.mobile && that.mobile.length == 11 && mobilereg.test(that.mobile)) {
                    ...
                    .then(function (response) {
                        ...
                        if (response.status = 200 && response.data.content.indexOf('账号已经存在') != -1) {
                            ...
                            that.showToastMsg('warn', '该手机号码已经存在', 2000);
                        }
                        else {
                            that.showToastMsg('warn', '该手机号码可用', 2000);
                ...
                if (that.mobile && that.mobile.length == 11 && mobilereg.test(that.mobile)) {
                    ...
                    if (!that.mobile_exist) {
                        if (!that.clickActive) {
                            ...
                            that.loading = true;
                            let url = '/v1/sms/global/generator/Register/' + that.mobile;
                            that.axios({
                                method: 'get',
                                url: url,
                                params: {
            ...
            codeBlur(){
                ...
                if(that.mobile_exist){
                    that.showToastMsg('warn', '手机号码已注册');
                    return false;
                }
                if(!that.code){
                ...
                let url = '/v1/sms/global/validate/Register/' + that.mobile;
                that.axios.get(url, {
                    params : {
                        code: that.code
                    }
                ...
                .then(function (response) {
                    ...
                    if(response.status = 200 && response.data.type == 'SUCCESS'){
                        ...
                        that.showToastMsg('correct', '验证成功');
                    }
                    else{
                        that.code_check = true;
            ...
            sbmit(){
                ...
                if(!this.loading) {
                    ...
                    this.axios({
                        ...
                        this.loading = false;
                        if (response.status == 200 && response.data.type == 'SUCCESS') {
                            this.showToastMsg('correct', '注册成功');
                            setTimeout(()=>{
                                this.$router.push({
